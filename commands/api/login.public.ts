...
export default {
  ...
  run: async (client, interaction) => {
    ...
    const { error: sessionError, data: session } = await tabnews.createSession({
      email,
      password
    });
    ...
    const { error: loginError, data: loggedUser } =
      await tabnews.getUserBySessionToken(session.token);

    if (loginError) {
      throw new NotFoundError({
        message:
          'Usuário não encontrado no sistema ou o token de sessão expirou.',
        action: 'Faça login novamente ou registre uma nova conta.',
        log: false
      });
    }

    const userObject = await user.getOneById(interaction.user.id);
